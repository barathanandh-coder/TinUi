...
4. [Low-Code UI Components (`LiveDataTable` & `AutoCRUD`)](#4-low-code-ui-components-livedatatable--autocrud)
...
TinPyUI decouples the UI layer from the data and platform layers. Any data source—relational table, document collection, WebSocket packet, or disk key-value cell—pipes into a reactive `Signal` or `LiveQuery` that updates the UI at 120 FPS without manual refresh loops.
...
|  - WebSockets & SSE Streams       -->   Platform Channels        -->   Win32 / WASM WebGPU    |
|                                                                                               |
+-----------------------------------------------------------------------------------------------+
```
...
The `tin.connect()` factory automatically identifies the protocol scheme and instantiates the matching database engine. If external drivers (`psycopg2`, `pymongo`) are not installed on the target machine, the engine seamlessly falls back to a zero-crash, embedded in-memory dialect engine.
...
# 3. SQLite / Local Disk File
sqlite_db = tin.connect("sqlite:///app_data.db")

# 4. In-Memory Ephemeral Engine
