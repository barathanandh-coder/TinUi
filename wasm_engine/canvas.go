...
func initNativeCanvas(container js.Value, backgroundType string) {
	...
	} else if backgroundType == "dynamic-color" || backgroundType == "gradient-flow" {
		startDynamicColorLoop(ctx, canvas, window)
	}
}
...
func startDynamicColorLoop(ctx js.Value, canvas js.Value, window js.Value) {
	var render js.Func
	hue := 0.0
	...
	render = js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		...
		gradient := ctx.Call("createLinearGradient", 0, 0, w, h)
		gradient.Call("addColorStop", 0, fmt.Sprintf("hsl(%.0f, 80%%, 15%%)", hue))
		gradient.Call("addColorStop", 1, fmt.Sprintf("hsl(%.0f, 80%%, 10%%)", hue2))
